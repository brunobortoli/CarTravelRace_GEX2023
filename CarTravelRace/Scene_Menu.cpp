...
void Scene_Menu::sRender() {
	...
	auto currentScene = m_game->getCurrentScene();

	if (currentScene != SceneID::MINS) {
	...
	for (size_t i{ 0 }; i < m_menuItems.size(); ++i)
	{
		...
		if (i == (m_menuItems.size() - 1) && currentScene == SceneID::MINS) {
			m_menuText.setPosition(32, 32 + (i + 2) * 96);
		}
		else {
